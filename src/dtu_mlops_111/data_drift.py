...
def validate_bucket_name(bucket_name: str) -> None:
    """
    Validates that a GCS bucket name follows Google Cloud Storage naming conventions.
    ...
    if not bucket_name:
        raise ValueError("Bucket name cannot be empty or contain only whitespace.")

    # Check length
    if len(bucket_name) < 3 or len(bucket_name) > 63:
        raise ValueError(f"Bucket name must be between 3 and 63 characters long. Got {len(bucket_name)} characters.")
    ...
    # Check for "google" anywhere in the name
...
def load_reference_data(data_path: Path = None, bucket_name: str = None, limit: int = None) -> pd.DataFrame:
    ...
    if data_path and data_path.exists():
        ...
        # Validate bucket name before using it
