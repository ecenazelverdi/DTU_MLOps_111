...
class CustomnnUNetPredictor(nnUNetPredictor):
    ...
    def __init__(
        ...
        if wandb.run is None:
            wandb.init(
                project="semantic_segmentation_nnunet_inference",
                config={"device": str(device), "tile_step_size": tile_step_size, "use_mirroring": use_mirroring},
            )
    ...
    def initialize_from_trained_model_folder(
        ...
        result = super().initialize_from_trained_model_folder(model_training_output_dir, use_folds, checkpoint_name)
        ...
        if wandb.run:
            wandb.config.update(
                {"model_folder": model_training_output_dir, "folds": str(use_folds), "checkpoint": checkpoint_name}
            )
    ...
    def predict_from_files(
        ...
        logger.info(
            f"Starting inference: {list_of_lists_or_source_folder} → {output_folder_or_list_of_truncated_output_files}"
        )
        logger.info(
            f"Preprocessing processes: {num_processes_preprocessing}, "
